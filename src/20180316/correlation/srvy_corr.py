...
td = min(list(set(np.round(np.diff(B_time), 5))))  # Uneven spacing!
...
print(f"data contains {len(chunk_start)} chunks")
...
MEDIAN_LEN = 150
rolling_med = np.empty(len(chunk_times) - 100)
rolling_time = np.empty(len(chunk_times) - 100)
for i in range(len(chunk_times) - 100):
...
ax[1].legend()
fmt = lambda t: dt.strftime(dt.utcfromtimestamp(t), "%H")
seconds = np.arange(time[0], time[-1] + 1, 1, dtype=int)
hours = seconds[np.nonzero(seconds % 3600 == 0)]
