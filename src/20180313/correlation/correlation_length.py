...
log.basicConfig(
    filename=f"{path}/correlation_length.log",
    level=log.INFO,
    format="%(asctime)s | %(name)s | %(levelname)s | %(message)s",
)


def exponential_fit(x, B):
    return np.exp(-x / B)
...
big_data_mag = np.linalg.norm(big_data, axis=1)
...
for bin in tqdm(bin_starts):
    ...
    correlated = correlated / correlated[correlation_x_axis == 0]  # Rescale to peak @ 1
    ...
    fitted, _ = curve_fit(
        exponential_fit, fit_correlated_x, fit_correlated, p0=(3), maxfev=int(1e6)
    )
    fitted_i = np.trapz(fit_correlated, fit_correlated_x)

    # Correction for length 1 array from curve_fit
    fitted = fitted[0]

    corr_lens.append(fitted)
    corr_lens_i.append(fitted_i)
    corr_times.append(time[len(time) // 2])
    correlation_time.append(fitted * ion_lims / meanv)
...
print(np.mean(correlation_time))
