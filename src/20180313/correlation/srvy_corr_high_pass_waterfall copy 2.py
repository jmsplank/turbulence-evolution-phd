...
def calc_v(start_t, end_t):
    return vx_i[get_index(time, start_t) : get_index(time, end_t)].mean()
...
for i, k in enumerate(lambdas.keys()):
    ax1.scatter(
        [calc_B(t[0], t[2]) for t in lambdas_times[k]],
        lambdas[k],
        cmap=cmap,
        marker="x",
        c=[k] * len(lambdas[k]),
...
for i, k in enumerate(lambdas.keys()):
    ax2.scatter(
        [calc_v(t[0], t[2]) for t in lambdas_times[k]],
        lambdas[k],
        cmap=cmap,
        marker="x",
        c=[k] * len(lambdas[k]),
